...
# # this loop calculates the flux detected at Earth for the M-dwarfs
...
# 	# calculate the flux at Earth's surface a distance, d, from the star (FOR M-DWARFS)
# 	d_cm = u.pc.to(u.cm,d)
# 	F_earth = F_stellar_surface*(R_eff/d_cm)**2
# 	Flux_Mdwarfs.append(F_earth)
# 	hdu.close()

# # this loop calculates the flux detected at Earth for the BSSs
# for i in np.arange(0,len(BSSs)):
# 	hdu = fits.open("/Users/mleveille/synth_spec/flux_data/"+str(BSSs[i]))
# 	F_stellar_surface = hdu[0].data*1e-8       # in erg/s/cm^2/angstrom
# 	R_eff = hdu[0].header['PHXREFF']           # in cm
	
# 	# calculate the flux at Earth's surface a distance, d, from the star (FOR BSSs)
# 	d_cm = u.pc.to(u.cm,d)
# 	F_earth = F_stellar_surface*(R_eff/d_cm)**2
...
def rebin_spec(wave, specin, wavnew):                                       # wavnew is the new wave grid
	...
	f = np.ones(len(wave))                                                  # preserve all flux at all wavelengths
